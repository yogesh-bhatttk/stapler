...
import { ensureFixture, WORD_TO_PDF, wordToPdfDocx } from './fixtures';
import { gotoTool, openApp } from './helpers';

/**
 * CNV-09 — the half of the acceptance criterion a unit test cannot reach: the
 ...
 *
 * Everything about the *conversion* is graded in `tests/unit/word-to-pdf.test.ts`
 ...
 * prevent.
 *
 * The `.docx` reaches the panel through the real file chooser (`openFiles` builds
 * an `<input type=file>` and clicks it), captured with Playwright's `filechooser`
 * event the same way `tool-flows.spec.ts` drives the Images-to-PDF picker.
 */
...
test.describe('CNV-09 — Word to PDF', () => {
  test('labels itself beta and refuses to save until the preview has run', async ({ page }) => {
    ...
    await openApp(page);
    // No PDF is opened first on purpose: the input is the `.docx`, and the tool
    ...
    await expect(page.getByText(/Choose a .docx file to convert first/)).toBeVisible();
    ...
    await chooseFixture(page, fixture, /Choose a \.docx file/);
    await expect(panel.getByText(/word-to-pdf\.docx/)).toBeVisible();
    ...
    await expect(outline.getByText(WORD_TO_PDF.h1)).toBeVisible();
    await expect(outline.getByText(/Table, 4 rows × 3 columns/)).toBeVisible();
    await expect(outline.getByText(/^Image, /)).toBeVisible();
    await expect(outline.getByText(`• ${WORD_TO_PDF.bullets[0]}`)).toBeVisible();
    ...
    expect(saved.suggestedFilename()).toBe('word-to-pdf.pdf');
  ...
  test('re-closes the gate when a different file is chosen after a preview', async ({ page }) => {
    ...
    await chooseFixture(page, fixture, /Choose a different \.docx/);
    await expect(save).toBeDisabled();
    await expect(page.getByText(/Preview the conversion first/)).toBeVisible();
    await expect(panel.getByText(/A preview is required before saving/)).toBeVisible();
  ...
  test('renders in the dark theme with no literal colour of its own', async ({ page }) => {
    ...
    const panel = page.getByRole('complementary', { name: /Word to PDF options/ });
    const heading = panel.getByRole('heading', { level: 1, name: 'Word to PDF' });
