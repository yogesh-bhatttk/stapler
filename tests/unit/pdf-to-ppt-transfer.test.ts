/**
 * CNV-12 — the image archive really is *transferred* into the convert worker,
 * not structured-cloned.
 *
 * The same claim CNV-08's audit found to be false as first written, re-proved for
 * this ticket's own call site. `operations.ts` used to call
 *
 ...
 * final `return [{ type: 'RAW', value }, transferCache.get(value) || []]`). So a
 * marker on a nested array is dropped and every image byte is copied — the exact
 * cost the signature exists to avoid.
 *
 * `convertPdfToPptx` has *more* to nest than its siblings did: alongside the
 * archive it passes the per-image report, the placement list and the option
 * flags. Putting the archive in that object would have been the natural shape and
 * the wrong one, so `buildPptx`'s signature keeps the `Uint8Array` at argument
 * position 1 and this test is what stops that being "tidied away".
 *
 ...
 * lives here rather than in `pdf-to-ppt.test.ts`: that file has to stub it so the
 * worker modules (which call `Comlink.expose` at import time) can load in Node.
 */
import { afterAll, describe, expect, it, vi } from 'vitest';
import * as Comlink from 'comlink';
import type { PageSlideData } from '../../src/core/convert/slides';
import type { ConvertJob, PptxBuildResult } from '../../src/core/workers/convert.worker';
import type {
  ExtractedImageEntry,
  ImagePlacementReport
} from '../../src/core/workers/process.worker';

/** One page with one line, enough to get past the "nothing on any slide" refusal. */
 ...
 * `pdf-to-ppt.test.ts`.
 */
const endpoint: Pick<ConvertJob, 'buildPptx'> = {
  async buildPptx(_pages, imageArchive, input): Promise<PptxBuildResult> {
    received.push({
...
const convertApi = Comlink.wrap<Pick<ConvertJob, 'buildPptx'>>(
...
vi.mock('../../src/core/workers', async () => {
  const renderApi = {
    ...
    extractPageSlide: async () => PAGES[0],
    closeDocument: async () => {}
  };
  const processApi = {
    ...
    // buffer, so `convertPdfToPptx` is free to hand it on — and this test can
    // watch what happens to it.
    extractImages: async () => ({ bytes: archive, entries: ENTRIES }),
    imagePlacements: async () => ({
...
describe('CNV-12 — the image archive is transferred, not copied', () => {
  it('detaches the archive buffer, which only a real transfer does', async () => {
    ...
    const result = await convertPdfToPptx(PDF, { includeText: true, includeImages: true });

    // The sending realm's buffer is gone: `postMessage` moved it. A structured
    // clone — which is what a version nesting it in the options object would
    // produce — would leave this at 1024.
    ...
    // The per-image report and the placement list ride alongside as normal
    // (cheap) clones — neither is a binary payload.
    expect(received[0].entries).toBe(1);
    expect(received[0].placements).toBe(1);
  ...
  it('passes null rather than an empty archive when images are switched off', async () => {
    ...
    await convertPdfToPptx(PDF, { includeText: true, includeImages: false });
    expect(received[0].byteLength).toBe(-1);
    expect(received[0].entries).toBe(0);
    expect(received[0].placements).toBe(0);
    // Nothing was extracted, so nothing was handed over either — and the
    // placement pass was not run.
    expect(archive.byteLength).toBe(3);
  }, 20_000);

  it('documents the regression: a marker nested in an object is silently dropped', async () => {
    // This is Comlink's behaviour, not Stapler's — pinned here so the reason
    // `buildPptx` keeps the `Uint8Array` at argument position 1, rather than
    // inside the (much larger) options object beside it, cannot be tidied away
