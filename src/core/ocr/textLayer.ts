...
/** WinAnsi-encodable subset. Anything else the standard font cannot show. */
 ...
 *
 * The font is embedded once for the whole document: Helvetica, not subsetted and
 * not embedded as a file, because the glyphs are never painted — only their widths
 * matter, and shipping a font program for text nobody sees would grow the file for
 * nothing.
 */
export async function addOcrTextLayerToDocument(
  ...
  for (const layer of withWords) {
    ...
    const { added, skipped } = drawInvisibleWords(page, font, fontKey, layer);
    wordsAdded += added;
