...
| Ticket | Status | Evidence / what is missing |
|---|---|---|
| OPS-01 merge | **Partial** | Multi-file merge with page-level reorder; mixed page sizes preserved, asserted on output bytes. **Correction:** bookmarks are still not preserved (pdf-lib has no outline API), but the limitation is now disclosed directly in `MergePanel.tsx` — this row's "nor is the limitation stated" was stale. The <8s-for-10×5MB budget is asserted in `tests/e2e/a11y-and-perf.spec.ts`. |
...
| Ticket | Status | Evidence / what is missing |
|---|---|---|
| ANN-01 overlays | **Partial** | `src/ui/tools/annotate/` (`AnnotatePanel.tsx`, `AnnotateOverlay.tsx`, `state.ts`) is a real annotation layer, separate from the SGN-02 signature-stamp layer. **Correction:** `AnnotationType` supports only `freehand \| highlight \| rectangle \| text` — sticky note and whiteout, both in the ticket's requirements, do not exist; this row previously claimed them and was stale. Also found and fixed this pass: the canvas had no keyboard path at all (violating the repo's full-keyboard-operation invariant) — Enter now adds a shape at a default size/position, arrows move it, Control+arrows resize a rectangle, Delete removes it, mirroring RedactOverlay's convention; the panel's labels (`tool.annotate.tool`/`.freehand`/`.highlight`/`.rectangle`/`.text`/`.color`/`.strokeWidth`) referenced i18n keys that existed in no locale at all, so they rendered as the literal key string for every user; freehand/highlight stroke width was an absolute pixel count against the on-screen canvas while position was normalised 0..1, so the same stroke exported at a different relative thickness than what was drawn depending on zoom (now normalised like x/y); and the colour swatches were spelled `'#' + 'FFEB3B'` specifically to dodge `scripts/check-tokens.mjs`'s raw-colour regex — moved to `doc-colors.ts` (the same audited exception `--doc-*` document colours use) with matching `--annotation-*` tokens in `tokens.css`. **Still missing:** undo/redo (only append-one/clear-all exist), sticky note, whiteout. |
...
| NFR-02 performance budgets | **Partial** | Five budgets now asserted for real: interactive <500ms, first thumbnail <1.5s, all-100-thumbnails-scrolled-through <6s, windowed mounting, merge 10×5MB <8s. Initial chunk is 152KB (52KB gzipped) against the 900KB budget, and consolidating five workers into three removed ~1.2MB of duplicated pdf.js and pdf-lib. **Two real bugs found and fixed this pass, not just "untested":** (1) `tests/fixtures/heavy.pdf` was supposed to be ~5MB of "noise" run through PNG/DEFLATE, but a plain linear congruential generator's output is not actually incompressible — `deflateSync` collapsed the intended 5.76MB buffer to ~55KB (a ~104x ratio impossible for genuinely random data), so every budget asserted against "10×5MB" was really exercising ~70KB files. Fixed by embedding the pixel data as a raw, unfiltered image XObject, whose output size no longer depends on how compressible its content is. (2) The "scroll to force rendering of later pages" step in the memory test (and the new all-100-thumbnails test) targeted `[data-testid="pagegrid-scroller"]`, which existed in no source file — `PageGrid.tsx`'s actual scroller had no test id at all, so the scroll silently never happened (guarded by an `isVisible()` check that just returns `false` for a non-existent locator rather than throwing). The test id is now on the real element. **Still untested:** peak memory and the <50ms main-thread rule. **The previous perf test asserted `tti < 5000` under a comment claiming the budget was 500ms** — a test that reports success while measuring nothing. |
...
2. ~~Resample the mask streams (CMP-03's remainder).~~ **Done in a later pass** —
   `encodeMask` (`render.worker.ts`) resamples the `/SMask` to the base image's new
   target and `rebuildCompressed` swaps it in whenever that shrinks it, verified in
   `tests/unit/process.test.ts`.
3. **Annotation layer (ANN-01) needs the same keyboard/i18n/undo pass every other
   tool already got.** Found in a later audit: the canvas had no keyboard path at
   all (fixed — Enter adds a shape, arrows move it, Delete removes it, mirroring
   RedactOverlay's convention); `tool.annotate.*` panel labels referenced i18n keys
   that did not exist in any locale, rendering literally (fixed); freehand/highlight
   stroke width was a raw pixel count against the on-screen canvas while position was
   normalised 0..1, so a stroke drawn at one zoom exported at a visibly different
   thickness than what was drawn (fixed — stroke width is now normalised the same
   way as x/y); and the swatch colours were spelled as `'#' + 'FFEB3B'` specifically
   to dodge `scripts/check-tokens.mjs`'s raw-colour regex (fixed — moved to
   `doc-colors.ts`, the same audited exception `--doc-*` uses, and named
   `--annotation-*` tokens in `tokens.css`). Still true: no undo/redo for
   annotations (only append-one/clear-all), and the documented scope (sticky notes,
   whiteout) is not yet built — only freehand, highlight, rectangle, and text exist.
