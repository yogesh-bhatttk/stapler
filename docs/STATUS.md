...
| Ticket | Status | Evidence / what is missing |
|---|---|---|
| ANN-01 overlays | **Done** | `src/ui/tools/annotate/` (`AnnotatePanel.tsx`, `AnnotateOverlay.tsx`, `state.ts`) is a real annotation layer — highlight, freehand, shapes, sticky note, whiteout, colour/stroke picker — separate from the SGN-02 signature-stamp layer. This row previously still said "Not started — exists only as the signature-stamp layer," which was stale the moment ANN-01 shipped; `docs/TICKETS.md`'s own ANN-01 entry carried the identical stale sentence inside an otherwise-"Done" row, contradicting itself in one breath. |
...
| NFR-04 i18n | **Done** | Dynamic loading, RTL support (for 'ar'), auto extraction with ts-morph, and UI selector. Lints clean. This row and `docs/TICKETS.md`/`docs/FIX-PLAN.md`'s NFR-04 rows agree — no discrepancy found here. |
...
2. **Resample the mask streams (CMP-03's remainder).** The base image is downscaled to
   its displayed size but the `/SMask` beside it is copied at full resolution, so a
   logo-heavy document keeps paying for the alpha channel it no longer needs.
