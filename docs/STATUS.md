...
| DOC-05 export pipeline | **Partial** | `useObjectStreams`, sensible names, correct order and rotation asserted on real bytes. Save-over-original is now offered: a document opened from exactly one writable file (via the picker path, tracked as `StaplerDoc.sourceHandle`) is asked, on every commit, whether to save over that file or save a new one — the choice is explicit rather than a silent default, and a merge/insert correctly loses the association since it is no longer "the same file". **Unverified by automation**: Playwright cannot drive the native file picker (documented in `tests/e2e/helpers.ts`), so the save-over path itself needs the QA-05 manual pass; type-checked and lints clean. QA-05's external-viewer check has also not been run. |
